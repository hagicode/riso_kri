...
<div id="QandA-1">
	...
	<dl>
		<dt>エンベロープとは？</dt>
		<dd>エンベロープは、移動平均線から一定の割合で上下にオフセットさせた線を表示するインジケータを指します。これにより、株価が移動平均線からどれだけ乖離しているかを視覚的に把握することが可能です。株価が移動平均線から大きく乖離した場合、最終的には移動平均線に戻る傾向があるため、この特性を利用して株価の反転ポイントを見つけたり、支持線や抵抗線として利用することができます。</dd>
		<dt>理想な乖離率とは？</dt>
		<dd>乖離率は銘柄によって異なり、効果的な乖離率を見つけるためには通常、目視による確認が必要です。しかし、本アプリでは標準偏差（σ）を用いて、統計的に株価反発が期待できる乖離率を上限値・下限値別々に算出します。これにより、上方乖離と下方乖離の傾向の違いを考慮することができます。</dd>
		<dt>マルチタイムフレーム</dt>
		<dd>Trading Viewは複数の時間足を並べて表示できるため、マルチタイムフレーム(MTF)で株価の分析が容易になります。短期足での乖離が大きくても、長期足ではそれほど大きな乖離ではない可能性があります。</dd>
	</dl>

	<h3>本アプリの機能</h3>
	<dl>
		<dt>①Trading Viewのエンベロープインジケータの自動生成</dt>
		<dd>直近のデータに基づき、各時間足で2σ以上の乖離率（発生頻度が4.5%未満）を持つエンベロープのインジケータコードを自動生成します。生成したコードはTrading Viewに直接貼り付けることで利用可能で、またテキストファイルとしてダウンロードも可能です。
        ...
        <br>※データ取得期間の設定は1分足=7日分,5分/15分足=60日分,1時間足=24カ月,日足=25年としていますが、銘柄や長期連休、ライブラリにより取得期間が短くなる場合があります。</dd>
    	<dt>④チャートイメージの生成</dt>
